...
[![Download stable UcsFPHub-2.0.47.zip](https://img.shields.io/badge/stable-UcsFPHub--2.0.47.zip-brightgreen)](https://github.com/Unicontsoft/UcsFiscalPrinters/releases/download/UcsFPHub-2.0.47/UcsFPHub-2.0.47.zip)
...
`ModelGroup`         | string  |          | Device model when it cannot be auto-detected by the protocol (e.g. `ZPL`)
`MaxPaymentLen`      | number  | Datecs   | Maximum number of symbols in a payment name (default `16`)
`UsePaymentTerminal` | bool    | Datecs   | Allow printing [Datecs Pay](https://www.datecspay.bg/) receipts/reports (default `Off`)
...
`CharacterSet`       | number  | Label    | Character set (default `6` for cp1251)
`DPI`                | number  | Label    | DPI of thermal printer (default `203`)
`Density`            | number  | Label    | Print density/darkness (default `-1` for don't set)
`DeviceWidth`        | number  | Label    | Width of device head in mm (default `57` or `108` for ZPL models)
`PaperWidth`         | number  | Label    | Width of loaded paper in mm (default `54` or `100` for ZPL models)
`TopMargin`          | number  | Label    | Label top margin (default `0`)
`LeftMargin`         | number  | Label    | Label teft margin (default `0`)
`RightMargin`        | number  | Label    | Label right margin (default `0`)
